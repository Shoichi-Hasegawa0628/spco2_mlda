...
class SendImg():
    ...
    def send_img_to_yolo(self, mode):
        if mode == '0':
            status = True
            count = 0
            rospy.wait_for_service('crop_img')
            img = np.array(Image.open(PRE_OBSERVATION + "observed_img.jpg"))
            ...
            observed_img = msg
            response = send_img(observed_img, count, mode)
            print(response)
            print("finish yolo")
            self.send_img_to_mlda(mode)
    ...
    def send_img_to_mlda(self, mode):
        if mode == '0':
            FOLDER = PRE_RESIZE
        else:
